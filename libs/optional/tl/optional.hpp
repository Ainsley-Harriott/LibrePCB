...
template <typename T>
inline QDataStream& operator<<(QDataStream& stream, const optional<T>& obj) noexcept {
    stream << (obj ? *obj : "null");
    return stream;
}

template <typename T>
inline QDebug operator<<(QDebug stream, const optional<T>& obj) noexcept {
    stream << QString("optional(%1)").arg(obj ? *obj : "null");
    return stream;
}

template <typename T>
inline uint qHash(const optional<T>& key, uint seed) noexcept {
    if (key) {
